...
dmd: $(RENDERER_LIB_OBJ) $(RENDERER_LIB_SRC)
	dmd $(RENDERER_LIB_OBJ) -unittest $(SRCFILES) -L-L/usr/local/lib -L-lslang -ofmain

ldc: $(RENDERER_LIB_OBJ) $(RENDERER_LIB_SRC)
	ldc2 -O5 -mattr=mmx -mattr=sse -mattr=sse2 -mattr=sse3 $(DFLAGS) -singleobj -release -inline $(SRCFILES) $(RENDERER_LIB_OBJ) -gc -L-L/usr/local/lib -L-lslang -ofmain

gdc: $(RENDERER_LIB_OBJ) $(RENDERER_LIB_SRC)
	gdc  -Ofast -march=native -mmmx -msse -msse2 -msse3 $(DFLAGS) $(SRCFILES) $(RENDERER_LIB_OBJ) -Wl,-ldl -Wl,-L/usr/local/lib -Wl,-lslang -omain

ll: $(RENDERER_LIB_LL) $(RENDERER_LIB_SRC)
	ldc2 -O5 -partially-inline-libcalls -singleobj -release -output-ll $(SRCFILES) -ofmain.ll
	#20% done
	...
	#80% done
	gcc -pg -march=native -Ofast main.o -o main $(LDFLAGS) -lphobos2-ldc -Wl,-Ofast -ldruntime-ldc -ldl -lpthread -lm
